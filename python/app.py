...
def cache_category_by_id():
    conn = dbh()
    sql = "SELECT * FROM `categories`;"
    redis_category = get_redis_category()
    with conn.cursor() as c:
        c.execute(sql)
        result = c.fetchall()
        for rs in result:
...
def get_category_by_id(category_id):
    ...
    redis_category = get_redis_category()
    category_str = redis_category.get(category_id)
    category = ast.literal_eval(category_str.decode())
    ...
    with conn.cursor() as c:
        ...
        try:
            ...
            if item_id > 0 and created_at > 0:
                ...
                sql = "SELECT IT.*, TE.*, SP.* FROM `items` as IT LEFT JOIN transaction_evidences as TE ON TE.item_id = IT.id LEFT JOIN shippings as SP ON TE.id = SP.transaction_evidence_id WHERE (IT.seller_id = %s OR IT.buyer_id = %s ) AND IT.status IN (%s,%s,%s,%s,%s) ORDER BY IT.created_at DESC, IT.id DESC LIMIT %s"
                c.execute(sql, [
                    user['id'],
                    user['id'],
                    Constants.ITEM_STATUS_ON_SALE,
            ...
            item_details = []
            hoge = loop.run_until_complete(queue_execution(items, item_details))
            #app.logger.info("================================")
            #app.logger.info(item_details)
            #app.logger.info("**********************************")
            #app.logger.info(hoge)
            item_details = sorted(item_details, key=lambda x: x["created_at"], reverse=True)
...
async def queue_execution(items, item_details):
    loop = asyncio.get_event_loop()
    queue = asyncio.Queue()
    ...
    async def fetch(q):
        while not q.empty():
            ...
            item["seller"] = to_user_json(seller)
            item["image_url"] = get_image_url(item["image_name"])
            result = to_item_json(item, simple=False)

            if "reserve_id" in item and item['reserve_id']:
                result["transaction_evidence_id"] = item["TE.id"]
                result["transaction_evidence_status"] = item["TE.status"]
                ssr = await loop.run_in_executor(None, api_shipment_status, get_shipment_service_url(), {"reserve_id": item["reserve_id"]})
                result["shipping_status"] = ssr["status"]
    ...
    tasks = [fetch(queue) for i in range(20)]
    ...
    try:
        ...
        with conn.cursor() as c:
            ...
            gather = asyncio.gather(
                asyncShipment(conn, buyer["address"], buyer["account_name"], seller["address"], seller["account_name"]),
                asyncPayment(conn, flask.request.json['token'], target_item['price'])
            )
            ...
            shipping_res = results[0].json()
            payment_res = results[1].json()
...
async def asyncShipment(conn, to_address, to_name, from_address, from_name):
    host = get_shipment_service_url()
    loop = asyncio.get_event_loop()
    try:
        func = functools.partial(requests.post, host+"/create", headers=dict(Authorization=Constants.ISUCARI_API_TOKEN), json=dict(to_address=to_address, to_name=to_name, from_address=from_address, from_name=from_name))
...
async def asyncPayment(conn, token, price):
    host = get_payment_service_url()
    loop = asyncio.get_event_loop()
    try:
        func = functools.partial(requests.post, host+"/token", json=dict(shop_id=Constants.PAYMENT_SERVICE_ISUCARI_SHOP_ID, api_key=Constants.PAYMENT_SERVICE_ISUCARI_API_KEY, token=token, price=price))
        res = await loop.run_in_executor(None, func)
        res.raise_for_status()
    except (socket.gaierror, requests.HTTPError) as err:
        conn.rollback()
